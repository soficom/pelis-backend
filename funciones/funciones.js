...
let funciones = {
    ...
    obtenerPeliculasPorId: function (id){
    ...
    obtenerPeliculasPorNombre : function(nombre){
        let peli = peliculas.find((p) => p.Title == nombre);
        if(peli != undefined){
            return peli;
        }else{
            return 'No existe pelicula';
        }
    },

    esPeliculaReto: (peli) => {return peli.Year < 2000},
    obtenerPeliculasReto : function(){
        let retros = peliculas.filter((p) => {return this.esPeliculaReto((p))});
        return retros;
    },
